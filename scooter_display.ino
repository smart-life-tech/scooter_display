...
int arc = ymax / 2;
int angle = 0;
char *label[] = {"speed", "COOLANT", "INTAKE", "VOLT"}; // some custom gauge labels
int labelXpos[] = {53, 45, 49, 53};                     // predefined x-position of a gauge label
#define potmeterPin A0                                  // simulate analogue value with potentiometer
int p, w, m, a = 10;
u8g_uint_t xx = 0;
...
void gauge(uint8_t angle)
{
   ...
   // u8g.drawBox(10, 100, 30, 20);
    u8g.drawBox(10, 100, map(progress, 0, 1023, 0, 124), 20); // actual gauge fill, remapped from 0-1023 to 0-124 (maximum rectangle size)
...
void loop(void)
{
    ...
    w = map(p, 0, 470, 0, 100); // map it between 0 and 100

    m = map(p, 0, 470, 0, 90); // map needle movement
